...
## 발표 일정 확인 — FinancialModelingPrep(FMP) 공식 API (2026-07-31, saveticker.com에서 교체)

**용도: "언제 물어봐야 하는지"(일정)와 "컨센서스가 얼마인지"를 한 번에 빠르게 훑는 것.** 실제값의 최종 확정 라벨은 여전히 1차 소스(FRED/ECOS/EIA 등) 기준이다.

**왜 saveticker.com이 아니라 FMP인가**: 이 스킬을 웹앱으로 옮기는 설계 검토 중(2026-07-30) `https://www.saveticker.com/robots.txt`를 직접 확인해보니 `ClaudeBot`을 이름으로 명시해 전체 차단(`Disallow: /`)하고 `Content-Signal: ai-train=no`로 자동화 접근을 명시적으로 거부하고 있었다. `claude-in-chrome`으로 참가자가 직접 여는 것과 자동 스크래퍼는 성격이 다르지만, 이름까지 지목해 거부한 사이트를 계속 안내하는 건 부적절하다고 판단해 FMP의 공식 `/economic_calendar` API로 옮겼다.

- 엔드포인트: `https://financialmodelingprep.com/api/v3/economic_calendar?from={YYYY-MM-DD}&to={YYYY-MM-DD}&apikey={FMP_API_KEY}`
- 무료 티어 하루 250회 한도 — macro-check는 세션당 한두 번만 호출하므로 충분하다.
- 응답에 **날짜/시간 + 국가 + 예상치(컨센서스) + 실제값 + 이전값**이 같이 나온다 — 아직 발표 전인 지표는 예상치만, 발표 후엔 실제값까지 채워진다.
- 일반 REST API라 WebFetch/curl로 바로 호출 가능(브라우저 자동화 불필요).
- **주의**: FMP 문서 페이지 자체가 자동 요청을 403으로 막아서, `event` 필드의 정확한 문자열(예: "CPI (YoY)")은 실제 응답을 받아 눈으로 확인하기 전까진 추측이다 — 첫 호출 결과를 참가자에게 보여주고 원하는 지표명과 매칭되는지 같이 확인한다.
- 이 API 값은 집계 소스라 `[2차]`. 판단 로그에 쓰는 "실제값"은 여기서 먼저 스캔하되, 라벨을 붙일 땐 가능하면 FRED/BLS/BEA/ECOS 1차로 교차 확인한다. 컨센서스(예상치)는 investing.com/TradingEconomics와 마찬가지로 `[2차]`+집계 신뢰도 캐비어트를 유지한다.

### 사용법 (macro-check 0.5번 단계에서)
1. `FMP_API_KEY`로 위 엔드포인트를 호출해 오늘 전후 구간을 조회한다.
2. **오늘 이전 ~ 지난 macro-check 이후** 구간에서 새로 실제값이 채워진 지표 = 오늘 판단 로그 대상 (SKILL.md 6번)
3. **앞으로 예정된 날짜** = 그 지표의 "다음 소스 액션" 사후 대조 예정일로 그대로 쓴다 (예: 다음 CPI는 응답에서 확인되는 다음 CPI 날짜)
4. 예상치(컨센서스) 칸은 SKILL.md 2번의 "구해지면 컨센서스" 부분을 여기서 바로 채울 수 있다 — investing.com 별도 조회가 필수가 아니게 된다.
5. `FMP_API_KEY`가 없으면 이 절차 전체를 건너뛴다 (SKILL.md 0.5번 참고) — 판단 자체는 2번부터 1차 소스만으로 계속 진행 가능하다.
...
| 미 근원CPI (식품·에너지 제외) | `CPILFESL` | 헤드라인 CPI와 같은 날 발표. SKILL.md 5번 "함께 보면 좋은 지표"에서 참고용으로만 쓴다(2026-07-31 결정 — 별도 판단 카드 없음) |
| 미 실업률 | `UNRATE` | |
